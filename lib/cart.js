...
module.exports = {
  ...
  rebuildCart: async (ctx, sessionId) => {
    ...
    cartItems.forEach(item => {
      const nacelleCartItem = {
        image: {
          thumbnailSrc: item.imageUrl
        },
        title: item.name,
        variant: {
          id: encodeBase64Id(`gid://shopify/ProductVariant/${item.variantId}`),
          price: item.price,
      ...
      ctx.store.dispatch('cart/addLineItem', nacelleCartItem)
    })
  }
}
