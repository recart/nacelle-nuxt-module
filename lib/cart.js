...
module.exports = {
  saveCart: async (cart, products, ctx) => {
    ...
    for (const lineItem of cart.lineItems) {
      ...
      // "productId" is a required field in the request so we need to return
      // but if we see many of these errors, we could accept requests without "productId" and extend the data on our backend
      if (!product) {
        reportError('Product not found', {
          lineItem,
